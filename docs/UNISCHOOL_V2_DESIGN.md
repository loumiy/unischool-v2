...
**The engine, as built (Phase 17).** An event is a row in `content/events.json`: an id, a kind, a weight, a cooldown in years, a `when` map of clauses, a sentence, a timeout in weeks, two or three choices with an optional price note, and the id of the choice it settles into unanswered. Clauses and effects are two closed vocabularies — a clause names a reading of the state (`backlogOver`, `triplesOver`, `adminShareOver`, `conditionUnder`, `rungAtLeast`, and so on) and an effect names a lever the engine can pull (`cash`, `endowment`, `confidence`, `mood`, `backlog`, `warmth`) — so a writer can reach nothing the engine has not agreed to, and a content file can never smuggle in code. Consequence weighting is arithmetic on the clauses: an event's weight is multiplied by a constant for each clause it carries, so an event the college had to earn outranks one that could happen to anybody. Subjects (`{building}`, `{faculty}`, `{class}`) are resolved once, at firing, so a sentence does not drift while it waits. The effects of a choice land through the levers and, for the students, through **mood**: a small signed term of satisfaction (§8.3's "current events") that the last few events leave behind and that fades over a year or two, so a run's weather is felt and then forgotten.

**The quiet is the feature, and it is measurable.** With the eleven proof events, a decade of a college minded properly asks about one question; a decade of a college whose maintenance has been cut to nothing asks seven or eight, and the ones it asks are the failures the administration caused — the roof, the heating, the backlog. The cadence is not the engine's dice, which offer an event most weeks; it is how few rows in the file have anything to say to a college that has done nothing wrong. That is consequence-weighted sourcing working, and it is also the measurement that sizes Phase 18: density comes from writing conditions that ordinary play reaches, not from raising the odds.
