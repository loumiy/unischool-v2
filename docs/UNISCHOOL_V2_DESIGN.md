...
**The schedule climbs (Phase 38).** A steward's teams won 77 titles in fifty years, because a winning team raised the athletics standing and the standing made the team stronger. The conference now rises to meet a rising programme. Every opponent plays above its own standing by `ATHLETICS_CLIMB` for each point of the college's athletics standing above 30, and a bigger athletics budget buys a harder schedule as well as a better team (the team keeps two fifths of the budget's edge). The rival raises its game for the meeting: it closes three fifths of any gap to the college. Titles fall to about one season in eight for the steward (11–14%), and a budget and venues still beat none. By the end of 1.1 the charters had pushed it back up: a land-grant steward, the college built for teams, won 24–30% of its seasons. Phase 52 raised `ATHLETICS_CLIMB` from 0.35 to 0.42, which holds that college at 19–23%, inside the 10–25% band (amended in Phase 52).
...
**Prices that scale with the college (amended in Phase 36).** An event's dollar amounts used to be absolute — a $200k roof was $200k in Year 3 and in Year 45 — and the review measured what that does: by the third decade no event cost a steward's college more than a rounding error, and the letters stopped being decisions. The catalogue is still written in founding-college dollars, but a price is now read against the college it is asked of: every `cash`, `endowment`, `debt` and `backlog` amount is multiplied, at firing, by the college's budgeted expenses over a reference budget (`EVENT_PRICE_REFERENCE_BUDGET`), never below one and capped (`EVENT_PRICE_SCALE_MAX`), and rounded to two significant figures so a letter says $1.4M rather than $1,387,112. Amounts under a floor (`EVENT_PRICE_FIXED_BELOW`) are not scaled — a small kindness costs what it costs. The scale is fixed when the event fires, like its subjects, so a price does not drift while it waits, and every dollar figure in the setup, the choices' notes and an ambition's stakes is rewritten at the same scale, so the words and the levers never disagree. Standing costs (dollars a year on a payroll) are not scaled, because a post is one salary and an office one office at any size; since Phase 51 neither is a note that quotes one, because the note used to say $1.4M a year while the payroll was charged $120k. Non-money levers are not scaled, except students gained or lost, which since Phase 51 are a share of the college like its money. The measure is §17's sting — the largest price a decade asks, as a share of that year's budget — which Phase 35 banded at 1–5%.

**Something to say about every system (Phase 51).** The 1.1 systems each have their own writing in the catalogue: adjuncts and contracts, capital projects (setbacks while one goes up, ribbon-cuttings when one opens, and a first day for each project the college can build), each founding charter, reputation (good word of mouth and bad press) and the last decades, so Years 40–50 read differently from Years 20–30. That needed six new readings: adjuncts on the roster; reputation, both ways; great projects standing; great projects going up; and the years since the newest opened. It also needed one new clause beside `needs`: `charters`, the founding charters an event can happen to. The same readings give two new ambitions (a college people recommend; a great project raised) and three new notes. Every event is held to the reachability harness, which now includes a college of each charter, some raising projects and some covering gaps with adjuncts.

**A handled event keeps its cooldown (amended in Phase 51).** An event settled by a seat (§9.2) is kept in the event history, marked as delegated, like one the President answered. Before, a delegated event left no history, so its cooldown never started: the same routine question could be settled by the same office every few weeks, and a staffed college heard a narrower world than an unstaffed one.
...
**The map at speed (Phase 52).** The release playtest profiled a Year-40 campus at 8× and found the map drawing every building, route and crowd again each week, because the estate wears every week and every layer was keyed on the placements. The layers that read the campus now hang on its layout: which building stands where, how big, and in what state of works. A building redraws when its wear crosses a stage, not when its backlog moves a dollar. The map draws a deferred copy of the state, so at 8× React draws the newest week it has time for and skips the rest; the chrome, the clock and the questions stay current. At 4× and 8× the map keeps its detail but drops its two dearest effects: the season's wash blends at normal instead of soft light, and the glint on the stream holds still. The glint is also drawn in short runs, because one animated path along the whole stream repaints most of the map every frame.
...
**The 1.1 review (Phase 52).** The second release playtest (`docs/reviews/1.1-playtest/REVIEW.md`) measured the guardrails on the finished 1.1. Saturation, demand, idle beats and tag overlap are in band on every run, and the steward's evening is back inside §2.2's spans. Two readings are still out of band, both with a reason. The first decade's sting stays under 1%, because the catalogue is priced for a founding college and the scale never goes below 1×. Cash cover dips below a quarter-year at Year 20 on the runs that are paying for a capital project that year, and recovers by Year 30. The idle-beat measure now counts a Board Meeting that deals the decade's list as a decision, because choosing from the list is one.
